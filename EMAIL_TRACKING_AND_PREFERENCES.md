...
- **follow_ups** (transactional) — lead follow-ups and review requests pass `preferenceStream: 'follow_ups'`; a recipient who opted out is skipped (logged as `skipped`/`unsubscribed`). The `send-follow-ups` cron additionally **cancels** every remaining pending queue item for that address on opt-out, so an unsubscribe stops the whole remaining sequence, not just the current send. Its opt-out is reached only via the follow-ups-mode unsubscribe link / one-click header on those emails — never the marketing cascade. A **won estimate** cancels the drip too: marking an estimate lead **Converted** in the admin Estimates Manager calls `cancelPendingFollowUps` (`src/lib/notify/cancelFollowUps.ts`), which flips that address's still-`pending` rows to `cancelled` so a customer who already said yes stops getting nudged - no extra step for the owner. Unlike the opt-out cancel, this one is **scoped to the lead-nurture drip types only** (`instant_ack`/`24h`/`48h`/`7d`, in `LEAD_NURTURE_FOLLOW_UP_TYPES`), so a repeat customer's pending review-request rows (`feedback_day0`/`feedback_day3`/`feedback_day7`) in the same queue survive. The email is matched case-insensitively with `ilike` (the queue stores the raw un-normalized address) with LIKE wildcards escaped so it stays literal. The cancel is best-effort: a failure never blocks the status change (that update already committed) - the owner gets a toast telling them to stop the emails from the Follow-ups admin. Scope is `converted` only; `lost` deliberately does not cancel. The **Follow-ups admin** (`/vaca-mgmt/follow-ups`) is the manual equivalent, and is person-centric. Its default **Active drips** tab lists one row per (person, sequence) that still has a pending email - built by the pure `buildActiveDrips` helper (`src/lib/followups/activeDrips.ts`), which groups the raw `follow_up_queue` rows by person + sequence (`nurture` vs `review`, kept separate because the queue is shared), soonest-next first, showing who they are, which drip, what's next (label + relative time), and the pending count. Each row has a **confirm-gated** **Stop drip** button that cancels only that person's clicked sequence via `cancelPendingFollowUps` (nurture → `LEAD_NURTURE_FOLLOW_UP_TYPES`, review → `REVIEW_REQUEST_FOLLOW_UP_TYPES`), so stopping one drip never touches the other sequence for that address. A second **All emails** tab keeps the raw per-email view (with status filters), where each pending/sent row's **Stop follow-ups** button calls the same helper scoped via `followUpSequenceTypes()` to the sequence the clicked row belongs to. Both set the affected rows to `cancelled`; review-request responses are still recorded separately from the Feedback admin. The same scoped stop is also reachable **estimate-side** without changing a lead's status: a **Stop follow-ups** button on the Send Estimate page (`/vaca-mgmt/send-estimate`) and on the Calculator Estimates lead detail (Status & Notes card in `EstimatesManager`) each call `cancelPendingFollowUps` at its default lead-nurture scope, so review-request emails are never touched. Lead follow-ups 2 (+24 h) & 3 (+48 h) also embed a **Home Care promo** (a "join Home Care" pitch wrapped in the `HC_PROMO_START`/`HC_PROMO_END` markers from `emailTemplates.ts`, with `lead_followup` UTM tags on the `/home-care` CTA); the cron strips just that block at send time — decided live via `isActiveHomeCareSubscriber()` because the body was frozen into `follow_up_queue` 24–48 h earlier — for recipients who are already active Home Care subscribers, and **fails open** (promo kept on any lookup error).
