...
| `GET /api/cron/monthly-newsletter` | Bearer `CRON_SECRET` | Navy/gold newsletter to affirmative marketing subscribers (Home Care + Buy+Remodel `active` rows), sent single-recipient through the `announcements` gate; per-issue dedup for safe re-runs; `?dryRun=1` counts recipients only. **Deliberately left unscheduled** (not in `vercel.json`) — manual trigger only until the owner confirms the first send |
...
- [ ] Apply the migrations above (including `20260805000000_email_prefs_follow_ups.sql`)
