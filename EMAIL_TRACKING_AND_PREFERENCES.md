...
| `GET/POST /api/admin/follow-ups` | admin middleware | Admin `follow_up_queue` access with the secret key (bypasses RLS, which grants only `anon`). `GET` lists all rows; `POST` handles `stop` (cancel a person's pending rows for one `sequence` — `nurture`/`review` — mapped server-side to the fixed type set), `cancel` (one row by `id`), and `resend` (re-queue an email now) |
