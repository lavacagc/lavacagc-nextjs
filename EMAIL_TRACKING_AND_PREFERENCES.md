...
- **follow_ups** (transactional) — lead follow-ups and review requests pass `preferenceStream: 'follow_ups'`; a recipient who opted out is skipped (logged as `skipped`/`unsubscribed`). The `send-follow-ups` cron additionally **cancels** every remaining pending queue item for that address on opt-out, so an unsubscribe stops the whole remaining sequence, not just the current send. Its opt-out is reached only via the follow-ups-mode unsubscribe link / one-click header on those emails — never the marketing cascade.

Every stream change is audited in `preference_events` (old/new value, actor `self | self_unverified | self_oneclick | admin | webhook | system`, actor detail, IP). Changes also sync the legacy status columns (`homeowners.status`, `newsletter_subscribers.status`) both ways. The legacy `/api/buy-and-remodel/unsubscribe` GET still writes through to `email_preferences` directly, but the legacy `/api/home-care/unsubscribe` GET is now **read-only**: it resolves the token and redirects to the preference center with a Home Care confirm prompt (never creating a row), so the actual leave happens through `applyUpdate` on the confirmed POST. A valid token presented by a mail-client prefetcher or corporate link scanner must not start an irreversible action, so the leave cannot begin on a GET. Re-enabling a stream only promotes `unsubscribed` legacy rows back to `active` — `pending` rows stay pending until they complete double opt-in verification.

**Home Care retention purge (Slice 8).** Turning the `home_care` stream off flips the `homeowners` status to `unsubscribed` for every actor, but only an **identity-proven, intentional leave** also deletes the homeowner's saved Home Details (`home_records`): actor `self` (a capability token only their inbox holds, acting through the preference center where the deletion is shown and confirmed) or `admin`. `self_unverified` (the tokenless `/unsub` by-email form, where the address is merely claimed), `self_oneclick` (RFC 8058 List-Unsubscribe, which turns off all streams with no surface to warn first), and `webhook`/`system` (Resend bounce/complaint auto-suppression, `contact.deleted`) suppress the mail but never purge. The staff `home_record_access_log` is deliberately **kept** on a leave (accountability data about staff, storing fact categories only); a true `homeowners`-row deletion still cascades it away via the FK. The purge never throws (the opt-out always sticks) and a real failure fires an internal alert (owner decision 2026-07-16; see `src/lib/homecare/retention.ts`).
