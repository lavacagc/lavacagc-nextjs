...
- **A draft resolves, for 24 hours.** Copy link is offered on a draft in the roster, so the link has to open - it is how an estimate is proof-read from the client's side, and how an admin hands a proposal over in a text message before Send.
  ...
  **The copy goes first and the refresh follows**, which is a browser constraint rather than a preference: a clipboard write is only permitted while the click's user activation is live, and WebKit drops that activation across an awaited network round trip, so refreshing in front of the write made it fail with `NotAllowedError` on Safari for every draft - the row the feature exists for.
  ...
  A refresh that fails still leaves the link copied, because the URL is what was asked for, but the toast stops promising the link opens and quotes what the server said - the 409s above are written for the admin, and collapsing them into one generic sentence told somebody whose colleague had just sent the proposal to send it again.
  ...
  Covered by `AC-S3-23` through `AC-S3-26` and by `B33`/`B34`/`B35` in the roster browser suite.
