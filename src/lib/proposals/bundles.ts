...
export function composeBundle(inputs: BundleInput[], name?: string): ComposedBundle | null {
  ...
  const members = inputs.flatMap((r) =>
    r.members && r.members.length > 0 ? r.members : [{ title: r.title, price_cents: r.priceCents }]);
  return {
 ...
 * (the member title is all we kept, so its verdict is recomputed - same
 * fail-safe: unknown restores locked).
 */
export function restoreMembers(members: BundleMember[]): {
  title: string; priceCents: number; optional: boolean; category: string;
}[] {
  return members.map((m) => {
    const verdict = categorizeLine(m.title);
    return { title: m.title, priceCents: m.price_cents, optional: verdict.optional, category: verdict.key };
  });
}
